...
export function ForecastCharts({
  ...
  // Prepare time-aware chart data format
  const chartData = forecastTrajectory.map((item, idx) => {
    const hours = item.operatingHours || (idx + 1) * 24;
    const label = item.targetDate ? `${item.targetDate}` : `+${hours}h`;
    return {
      label,
    ...
    <div className="rounded-2xl border border-[#1B1E2B] bg-[#0B0C12] p-5 font-mono space-y-4 shadow-xl">
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-[#181B28]">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-xs font-bold text-white uppercase tracking-wider">
            PROJECTED DEGRADATION TRAJECTORY (OPERATING HOURS & TARGET DATES)
          </h3>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between text-xs text-[#94A3B8] px-1">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-[#64748B]">(Time-Aware XGBoost Operating Hours Projection)</span>
        </div>

        <div className="flex items-center gap-4 text-[11px]">
          <span className="flex items-center gap-1.5 text-[#FF1744]">
            <span className="w-4 h-0.5 bg-[#FF1744] border border-dashed" />
            <span>Operating Threshold Limit ({limitValue} {metric.unit})</span>
          </span>

          {violatingSensor === selectedMetric && (
            <span className="px-2 py-0.5 rounded bg-[#FF1744]/15 border border-[#FF1744]/30 text-[#FF1744] font-bold text-[10px] flex items-center gap-1">
              <AlertTriangle size={11} /> ELEVATED DEGRADATION
            </span>
          )}
        </div>
      </div>
      ...
      <div className="h-64 w-full pt-2">
        {chartData.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-xs text-[#475569]">
            ...
            <p>No forecast trajectory available. Execute live inference to generate time-aware maintenance forecast.</p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#181B28" />
              <XAxis dataKey="label" stroke="#475569" fontSize={9} interval={Math.floor(chartData.length / 6)} />
